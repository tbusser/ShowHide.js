...
(function(root, factory) {
	...
	exports.options = {
		...
		attrTextClosed : 'data-sh-text-closed',
		attrTextOpened : 'data-sh-text-opened',
	...
	/**
	 * This method will check if the trigger element has the aria-controls
	 * attribute. When the attribute is missing or if the attribute references
	 * a non-existing ID the function will try to set the attribute on the
	 * trigger element and have it reference the content element
	 */
	function _checkAriaControlsAttribute(trigger, triggerId) {
		var contentElement,
		    contentId,
		    generateRandomId = function() {
				return 'sh-' + new Date().getTime();
			}

		// Check if the trigger element already has an aria-controls attribute
		if (trigger.hasAttribute('aria-controls')) {
			// Try to get the element which is controlled by the trigger
			if (document.getElementById(trigger.getAttribute('aria-controls')) != null) {
				// We found an element with the specified ID, we're done
				return;
			}
			// At this point we know the trigger does have an aria-controls
			// attribute but it doesn't reference an existing ID. We will
			// continue running the function to try and fix this
		}

		// Try to find the content element which belongs to the trigger
		contentElement = document.querySelector('[' + options.attrContent + '="' + triggerId + '"]');
		// If no content element could be located there is nothing else to do
		...
		// Check if the content element is missing an id
		if (contentId == null || contentId === '') {
			// Generate a random ID
			contentId = generateRandomId();
			// Keep generating random IDs till we've created one that doesn't
			// return an element
			while (document.getElementById(contentId) != null) {
				contentId = generateRandomId();
			}
			// Assign the unique ID to the content element
			contentElement.id = contentId;
		}

		// Set the aria-controls attribute with the random generated ID as its
		// value
	...
	function _closeItem(id, skipTransition) {
		var container = document.querySelector('[' + options.attrContainer + '="' + id + '"]'),
		    content = document.querySelector('[' + options.attrContent + '="' + id + '"]'),
		...
		// Update the text of the trigger to relfect the new state
		_updateTriggerCaption(trigger, false);
	...
	*/
	...
	function _onTriggerEventHandler(event) {
	...
	function _openItem(id, skipTransition) {
		...
		// Update the text of the trigger to relfect the new state
		_updateTriggerCaption(trigger, true);
	...
	function _updateTriggerCaption(trigger, isOpen) {
		// Depending on whether the detail view is opened or closed we will have
		// to use a different attribute to retrieve the trigger caption
		var attributeName = (isOpen) ? options.attrTextOpened : options.attrTextClosed,
		    captionElement;

		// Check if the trigger itself has the attribute we're looking for
		if (trigger.hasAttribute(attributeName)) {
			// The trigger element has the attribute, we will use this as the
			// caption element
			captionElement = trigger;
		} else {
			// Check if the trigger has a chile element which has the attribute
			// we're looking for
			captionElement = trigger.querySelector('[' + attributeName +  ']');
		}

		// If we were unable to find an element with the attribute we need there
		// is nothing left for us to do
		if (captionElement == null) {
			return;
		}

		// 1: Get the value of the attribute from the element we've determined
		//    has the attribute
		// 2: Create a text node with the text from the attribute
		var caption = captionElement.getAttribute(attributeName),      /* [1] */
			text = document.createTextNode(caption);                   /* [2] */
		// Display the caption in the element
		captionElement.textContent = text.textContent;
	}
	...
	exports.prototype = {
		...
		init: function(onInitCallback, context) {
			...
			for (; index < ubound; index++) {
				...
				// Check the aria-controls attribute of the trigger element
				_checkAriaControlsAttribute(trigger, triggerId);
				// Add a click handler for the trigger, this will allow the item to be opened
				// and closed
				trigger.addEventListener(options.triggerEvent, _onTriggerEventHandler);
		...
		replaceTrigger: function(id, newElement) {
			...
			// to the new trigger
			trigger.removeEventListener(options.triggerEvent, _onTriggerEventHandler);
			newElement.addEventListener(options.triggerEvent, _onTriggerEventHandler);

			// Make sure we keep the aria attributes
			newElement.setAttribute('aria-expanded', trigger.getAttribute('aria-expanded'));
			newElement.setAttribute('aria-controls', trigger.getAttribute('aria-controls'));
