/**
 * ShowHide.js - version 1.2 / jQuery
 *
...
(function(root, factory) {
	...
	function _closeItem(id, skipTransition) {
		...
		$(container).removeClass(options.cssOpened);
		$(container).addClass(options.cssClosed);

		if (trigger.hasAttribute(options.attrTextClosed)) {
			var text = document.createTextNode(trigger.getAttribute(options.attrTextClosed));
		...
		if (initialized && applyTransition) {
			...
			$(container).on(options.transitionEnd, _onTransitionEndHandlerContainer);
			...
			setTimeout(function() {
				...
				$(container).addClass(options.cssTransition);
			...
			setTimeout(function() {
				...
				if (options.transitionEnd === NO_TRANSITIONS) {
					...
					$(container).add(options.cssTransition);
				}
			}, 10);
		}
	}
	...
	function _getCollapsedHeight(container) {
		...
		if (trigger != null) {
			...
			if ($(container).css('position') !== 'absolute') {
				...
				result += parseInt($(container).css('margin-bottom'), 10) + parseInt($(container).css('margin-top'), 10);
			}
		}

		if ($(container).css('box-sizing') === 'border-box') {
			result += parseInt($(container).css('border-bottom-width'), 10) + parseInt($(container).css('border-top-width'), 10);
		}
	...
	function _onClickHandlerTrigger(event) {
		...
		if ($(container).hasClass(options.cssClosed)) {
			_openItem(itemId);
		} else {
			_closeItem(itemId);
		}
	...
	*/
	function _onTransitionEndHandlerContainer(event) {
		...
		$(container).off(options.transitionEnd, _onTransitionEndHandlerContainer);
		_makeContainerSane(container);
	}
	...
	function _openItem(id, skipTransition) {
		...
		$(container).removeClass(options.cssClosed);
		$(container).addClass(options.cssOpened);

		trigger.setAttribute('aria-expanded', 'true');
		content.setAttribute('aria-hidden', 'false');

		// Check if the trigger has a text for when the detail view is open
		if (trigger.hasAttribute(options.attrTextOpened)) {
			// Replace the trigger text for the text when the detail view is open
			var text = document.createTextNode(trigger.getAttribute(options.attrTextOpened));
			trigger.textContent = text.textContent;
		}
		...
		$(container).removeClass(options.cssTransition);
	...
	function _makeContainerSane(container) {
		...
		if ($(container).hasClass(options.cssTransition)) {
			$(container).removeClass(options.cssTransition);
	...
	exports.prototype = {
		...
		hideDetailView: function(id, skipTransition) {
			...
			if (container == null || $(container).hasClass(options.cssClosed)) {
		...
		init: function(onInitCallback, context) {
			...
			for (; index < ubound; index++) {
				...
				// and closed
				$(trigger).on('click', _onClickHandlerTrigger);
			...
			$(document.documentElement).removeClass('no-read-more');
		},
		...
		replaceTrigger: function(id, newElement) {
			...
			// to the new trigger
			$(trigger).off('click', _onClickHandlerTrigger);
			$(newElement).on('click', _onClickHandlerTrigger);
		...
		showDetailView: function(id, skipTransition) {
			...
			// Make sure we found the container and it is not yet open
			if (container == null || $(container).hasClass(options.cssOpened)) {
				// Either the container was not found or it already contains the
				// class opened; either way we stop
				return;
			}
